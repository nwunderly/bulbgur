...
import aiohttp
from auth import API_KEY, EMAIL, PASSWORD, SECRET_KEY
...
async def dash(request: Request):
    ...
    if len(session) != 0:
        if session["token"] == app.token:
            ...
            return templates.TemplateResponse(
                "dash.html", {"request": request, "images": images}
            )
        else:
            raise HTTPException(401)
    else:
...
async def new_short_url(request: Request):
    if (
        request.headers.get("x-authorization") == API_KEY
...
async def new_file_upload(request: Request):
    if request.headers.get("x-authorization") == API_KEY:
        ...
        if extension not in allowed_extensions:
            raise HTTPException(
                status_code=415,
                detail=f"File type not supported. Allowed extensions: {allowed_extensions}",
            )
